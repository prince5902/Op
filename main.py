...
# =======================
#    FLASK APP (Render 24/7 Port Fix)
# =======================
...
def run_flask():
    try:
        # Render স্বয়ংক্রয়ভাবে PORT এনভায়রনমেন্ট ভেরিয়েবল প্রদান করে, না পেলে ডিফল্ট 10000 বা 8080 ব্যবহার হবে
...
async def send_force_join_message(update_or_query, context, lang, user_id):
    ...
    try:
        ...
        print(f"[FORCE JOIN MSG ERROR] {e}", flush=True)
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"[ADMIN CMD ERROR] {e}", flush=True)
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if text == strings["menu_my_account"]:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    ...
                    InlineKeyboardButton("🇮🇳 हिन्दी", callback_data="change_lang_hi")
                ]
            ])
        ...
        print(f"[TEXT MSG ERROR] {e}", flush=True)
...
async def send_active_number_card(update: Update, context: ContextTypes.DEFAULT_TYPE, service: str = None, query=None):
    try:
        ...
        print(f"[NUMBER CARD ERROR] {e}", flush=True)
...
async def show_manage_force_channels(query, context):
    try:
        ...
        print(f"[FORCE CHAN ERROR] {e}", flush=True)
...
async def handle_callback_queries(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if query.data == "get_new_number":
            ...
            await query.message.reply_text(
                "💬 Send Refer Bonus and Bot Username separated by a bar (|).\nExample: <code>15 | MyBotUsername</code>",
                parse_mode="HTML"
            )
            ...
            await query.message.reply_text(
                "💬 Send Methods and Min Withdraw separated by a bar (|).\nExample: <code>BKash, Nagad | 50</code>",
                parse_mode="HTML"
            )
...
async def handle_withdraw_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"[WITHDRAW INPUT ERROR] {e}", flush=True)
...
async def process_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if state == "waiting_for_otp_channel":
            ...
            if is_admin:
                if not chan_link:
                    chan_link = f"https://t.me/{chat.username}" if chat.username else "https://t.me"
                ...
                await update.message.reply_text(f"✅ <b>OTP Send Channel Saved!</b>\n📡 Name: <b>{chat.title}</b>", parse_mode="HTML")
            else:
                await update.message.reply_text("❌ Bot is NOT an Administrator inside this channel.", parse_mode="HTML")
            ...
            save_config(config)
            context.user_data["admin_state"] = None
            await update.message.reply_text("✅ <b>Official Channel Link Saved!</b>", parse_mode="HTML")
            ...
            if is_admin:
                ...
                config = load_config()
                force_list = config.get("force_channels", [])
                if not any(c["id"] == chat.id for c in force_list):
                    force_list.append({"id": chat.id, "title": chat.title or chan_id, "link": chan_link})
                    config["force_channels"] = force_list
                    save_config(config)
                
                context.user_data["admin_state"] = None
                await update.message.reply_text(f"✅ <b>Force Channel Added:</b> {chat.title}", parse_mode="HTML")
            else:
                await update.message.reply_text("❌ Bot must be an Admin in that channel.", parse_mode="HTML")
            ...
            save_config(config)
            context.user_data["admin_state"] = None
            await update.message.reply_text("✅ <b>Refer settings saved!</b>", parse_mode="HTML")
            ...
            save_config(config)
            context.user_data["admin_state"] = None
            await update.message.reply_text("✅ <b>Withdraw settings saved!</b>", parse_mode="HTML")
            ...
            save_config(config)
            context.user_data["admin_state"] = None
            await update.message.reply_text("✅ <b>Support Link saved!</b>", parse_mode="HTML")
    except Exception as e:
        print(f"[ADMIN INPUT ERROR] {e}", flush=True)

# =======================
#    BACKGROUND TASKS (Crash-Proof)
# =======================
...
def main():
    ...
    # ব্যাকগ্রাউন্ডে Flask সার্ভার চালু (Render Port বাইন্ডিং নিশ্চিত করার জন্য)
    flask_thread = Thread(target=run_flask)
    flask_thread.daemon = True
    flask_thread.start()

    # বট ক্রাশ রোধী করতে এবং নেটওয়ার্ক লুপ ধরে রাখতে Auto-reconnect লজিক
    while True:
        try:
            ...
            print("🤖 Bot started successfully and running with Polling...", flush=True)
            app.run_polling(drop_pending_updates=True, allowed_updates=Update.ALL_TYPES)
        except Exception as e:
            print(f"[CRITICAL BOT ERROR] Bot crashed with error: {e}. Restarting in 5 seconds...", flush=True)
            import time
            time.sleep(5)
